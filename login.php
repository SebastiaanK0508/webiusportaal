...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (is_login_locked_out($gebruikersnaam)) {
        $error = 'Te veel mislukte pogingen. Probeer het over 15 minuten opnieuw.';
    } else {
        $stmt = $pdo->prepare("SELECT id, website_id, role, username, password_hash, is_active FROM users WHERE username = ? LIMIT 1");
        $stmt->execute([$gebruikersnaam]);
        $user = $stmt->fetch();

        if ($user && (int)$user['is_active'] === 1 && password_verify($wachtwoord, $user['password_hash'])) {
            if ($user['role'] === 'client' && empty($user['website_id'])) {
                $error = 'Dit account is aan geen enkele website gekoppeld. Neem contact op met de beheerder.';
                register_login_attempt($gebruikersnaam, false);
            } else {
                register_login_attempt($gebruikersnaam, true);
...
<html lang="nl">
<head>
    <title>Inloggen - Webius Portaal</title>
