...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action']) && $_POST['ajax_action'] == 'update_order') {
    ...
    if ($type === 'categories') {
        $stmt = $pdo->prepare("UPDATE categories SET sort_order = ? WHERE uuid = ?");
    } else {
        $stmt = $pdo->prepare("UPDATE products SET sort_order = ? WHERE uuid = ?");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add_category') {
    ...
    $stmt = $pdo->prepare("INSERT INTO categories (uuid, name, sort_order) VALUES (UUID(), ?, ?)");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_categories') {
    foreach ($_POST['categories'] as $cat_uuid => $cat_data) {
        $stmt = $pdo->prepare("UPDATE categories SET name = ? WHERE uuid = ?");
        $stmt->execute([$cat_data['name'], $cat_uuid]);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'add_product') {
    ...
    $stmt = $pdo->query("SELECT MAX(sort_order) FROM products");
    $max = $stmt->fetchColumn();
    $sort_order = $max ? $max + 1 : 1;
    $stmt = $pdo->prepare("INSERT INTO products (uuid, category_id, title, description, price, sort_order) VALUES (UUID(), ?, ?, ?, ?, ?)");
    $stmt->execute([$category_id, $title, $description, $price, $sort_order]);
...
if (isset($_GET['delete_prod'])) {
    $stmt = $pdo->prepare("DELETE FROM products WHERE uuid = ?");
...
<html lang="nl">
<head>
    <title>Beheer Diensten & Categorieën</title>
    ...
    <div class="max-w-7xl p-8 mx-auto">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
            <div>
                ...
                <p class="text-gray-500 mt-1">Beheer hier je categorieën en de bijbehorende behandelingen.</p>
            </div>
            <div class="flex items-center gap-4 mt-4 md:mt-0">
                ...
                <a href="prijzen.php" class="bg-pink-100 text-pink-700 font-bold py-2 px-6 rounded-full hover:bg-pink-200 transition-colors">
                    Bekijk publiek &rarr;
                </a>
            </div>
        </div>
        ...
        <?php if(isset($_GET['msg'])): ?>
            <div class="mb-8 p-4 rounded-lg font-medium 
                <?php 
                    echo (strpos($_GET['msg'], 'deleted') !== false) 
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody id="categories-list" class="divide-y divide-gray-200 bg-white">
                                        <?php foreach ($categories as $index => $cat): ?>
                                            <tr data-id="<?php echo htmlspecialchars($cat['uuid']); ?>">
                                                ...
                                                <td class="px-4 py-3">
                                                    <input type="text" name="categories[<?php echo htmlspecialchars($cat['uuid']); ?>][name]" value="<?php echo htmlspecialchars($cat['name']); ?>" class="w-full border-transparent hover:border-gray-300 rounded p-1.5 focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition-colors">
                                                </td>
                                                <td class="px-4 py-3 text-right">
                                                    <a href="?delete_cat=<?php echo htmlspecialchars($cat['uuid']); ?>" onclick="return confirm('Weet je het zeker? LET OP: Alle behandelingen in deze categorie worden ook verwijderd!');" class="text-red-500 hover:text-red-700 font-medium text-sm">Verwijder</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                ...
                <div class="lg:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <?php if(empty($categories)): ?>
                        <div class="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
                            <p class="text-sm text-yellow-700">Je moet eerst een categorie aanmaken in de sectie hierboven.</p>
                        </div>
                    <?php else: ?>
                        <form method="POST" action="productbeheer.php">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="category_id" required class="w-full border-gray-300 rounded-md border p-2 focus:border-pink-500 focus:ring-pink-500 bg-white">
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?php echo htmlspecialchars($cat['uuid']); ?>"><?php echo htmlspecialchars($cat['name']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" step="0.01" name="price" required class="w-full border-gray-300 rounded-md border p-2 focus:border-pink-500 focus:ring-pink-500">
                            </div>
                                ...
                                <div class="overflow-x-auto border rounded-lg border-gray-200">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        ...
                                        <tbody class="products-list divide-y divide-gray-200 bg-white">
                                            <?php if (!empty($productsByCategory[$cat['uuid']])): ?>
                                                <?php foreach ($productsByCategory[$cat['uuid']] as $index => $p): ?>
                                                    <tr data-id="<?php echo htmlspecialchars($p['uuid']); ?>" class="hover:bg-gray-50">
                                                        <td class="drag-column hidden px-4 py-3 drag-handle text-gray-400 hover:text-pink-500 bg-pink-50">☰</td>
                                                        <td class="order-column px-4 py-3 text-sm text-gray-500 font-bold"><?php echo $index + 1; ?></td>
                                                        ...
                                                        <td class="px-4 py-3 text-right">
                                                            <a href="?delete_prod=<?php echo htmlspecialchars($p['uuid']); ?>" onclick="return confirm('Weet je zeker dat je deze dienst wilt verwijderen?');" class="text-red-500 hover:text-red-700 font-medium text-sm">Verwijder</a>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            toggleBtn.addEventListener('click', function() {
                ...
                // Toggle Categorie Dragging
                if(sortableCategories) sortableCategories.option("disabled", !isDragMode);
                
                // Toggle Product Dragging voor alle categorieën
