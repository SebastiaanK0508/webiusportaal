...
if (isset($_POST['ajax_mark_read']) && isset($_POST['uuid'])) {
    $stmt = $pdo->prepare("UPDATE contact_messages SET is_read = 1 WHERE uuid = ?");
    $stmt->execute([$_POST['uuid']]);
    exit('success');
}
...
if (isset($_POST['update_contact_content'])) {
    $pdo->query("UPDATE site_content SET is_visible = 0 WHERE section_key LIKE 'contact_%'");
...
if (isset($_GET['delete_message'])) {
    $stmt = $pdo->prepare("DELETE FROM contact_messages WHERE uuid = ?");
    $stmt->execute([$_GET['delete_message']]);
    $message = "Het bericht is definitief verwijderd.";
    $msg_type = 'delete';
}
$contact_content = $pdo->query("SELECT * FROM site_content WHERE section_key LIKE 'contact_%' ORDER BY id ASC")->fetchAll();
    ...
    <div class="max-w-6xl mx-auto px-4 mt-8">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            ...
            <div class="flex gap-3">
                <a href="contact.php" target="_blank" class="text-pink-600 hover:text-white font-bold bg-white hover:bg-pink-600 px-6 py-2 rounded-full shadow border border-pink-100 transition-all">Bekijk contactpagina &rarr;</a>
            </div>
        </div>
        ...
        <?php if ($message): ?>
            ...
            if ($msg_type === 'delete') {
                ...
                $btn_class = 'text-red-500 hover:text-red-800 hover:bg-red-100';
                ...
                $btn_class = 'text-green-500 hover:text-green-800 hover:bg-green-100';
            ...
            <div id="alert-message" class="<?php echo $bg_class; ?> border-l-4 p-4 mb-6 shadow-sm rounded-r-lg flex justify-between items-center transition-all duration-500 transform translate-y-0 opacity-100">
                ...
                <button onclick="closeAlert()" class="<?php echo $btn_class; ?> rounded-lg p-1.5 focus:outline-none transition-colors">
            ...
            <?php else: ?>
                <div class="grid gap-4">
                    <?php foreach ($contact_messages as $msg): 
                        ...
                        // Visuele styling: roze randje en witte achtergrond als hij ongelezen is, anders grijs.
                        $card_class = $is_unread ? 'border-l-4 border-pink-500 bg-white shadow-md' : 'border border-gray-200 bg-gray-50';
                    ?>
                        <div id="card-<?php echo $msg['uuid']; ?>" class="rounded-xl p-5 relative flex flex-col md:flex-row gap-4 items-start md:items-center transition-all duration-300 <?php echo $card_class; ?>">
                            ...
                            <div class="flex items-center gap-4 w-full md:w-1/3">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <?php if($is_unread): ?>
                                            <span id="badge-<?php echo $msg['uuid']; ?>" class="bg-pink-500 text-white text-[10px] uppercase font-bold px-1.5 py-0.5 rounded">Nieuw</span>
                                        <?php endif; ?>
                                    </div>
                            ...
                            <!-- Aangepast: Alleen onderwerp, email en telefoon laten zien in de lijst -->
                            ...
                            <div class="w-full md:w-auto flex md:ml-auto gap-2 mt-2 md:mt-0">
                                ...
                                <button onclick="confirmDelete('<?php echo $msg['uuid']; ?>', '<?php echo htmlspecialchars(addslashes($msg['name'])); ?>')" class="text-gray-400 hover:text-red-600 bg-white border border-gray-200 hover:border-red-200 hover:bg-red-50 p-2 rounded-lg transition-colors" title="Verwijderen">
                ...
                <div class="grid md:grid-cols-2 gap-6">
                    <?php foreach ($contact_content as $item): ?>
                        <?php 
                            ...
                            $friendly_name = isset($contact_labels[$key]) ? $contact_labels[$key] : ucfirst(str_replace('_', ' ', $key));
                        ?>
                        <div class="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm relative group">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <label class="font-bold text-gray-800 text-sm block"><?php echo $friendly_name; ?></label>
                                    <span class="text-[10px] text-gray-400 font-mono mt-0.5 block"><?php echo $key; ?></span>
                                </div>
                            ...
                            <?php if (strpos($key, 'title') !== false): ?>
                                <input type="text" name="content[<?php echo $key; ?>]" value="<?php echo htmlspecialchars($item['content_text']); ?>" class="w-full px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-pink-400 text-sm transition-all shadow-inner">
                            <?php else: ?>
                                <textarea name="content[<?php echo $key; ?>]" rows="4" class="w-full px-4 py-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-400 focus:border-pink-400 text-sm transition-all shadow-inner"><?php echo htmlspecialchars($item['content_text']); ?></textarea>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 transform scale-95 opacity-0 transition-all duration-300 flex flex-col max-h-[90vh]" id="viewModalContent">
                ...
                <div class="flex justify-between items-center p-6 border-b border-gray-100">
                    <div class="flex items-center gap-4">
                        <div id="view-avatar" class="w-12 h-12 rounded-full bg-pink-100 text-pink-600 flex items-center justify-center font-bold text-xl">
                            <!-- Initiaal komt hier -->
                        </div>
                        <div>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full mx-4 transform scale-95 opacity-0 transition-all duration-300" id="deleteModalContent">
                ...
                <div class="text-center">
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 justify-center">
                        <button onclick="closeDeleteModal()" class="w-full sm:w-auto px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-bold hover:bg-gray-50 transition-colors">
                            Annuleren
                        </button>
                        <a href="#" id="confirmDeleteBtn" class="w-full sm:w-auto px-6 py-3 bg-red-600 rounded-lg text-white font-bold hover:bg-red-700 shadow-md shadow-red-600/30 transition-all">
        ...
        <script>
            // Bericht Bekijken Modal & AJAX Gelezen Status
            function openViewModal(msgData) {
                ...
                // --- AJAX: Markeer als gelezen op de achtergrond ---
                const badge = document.getElementById('badge-' + msgData.uuid);
                if (badge) {
                    // Stuur request naar de server (bovenin dit bestand)
                    let formData = new FormData();
                    formData.append('ajax_mark_read', '1');
                    formData.append('uuid', msgData.uuid);
                    ...
                    fetch('contactbeheer.php', {
                        ...
                        if (response.ok) {
                            // Update de UI live!
                            badge.remove(); // Verwijder "Nieuw" label
                            
                            // Verander het roze kader terug naar een normale grijze
                            const card = document.getElementById('card-' + msgData.uuid);
                            card.classList.remove('border-l-4', 'border-pink-500', 'bg-white', 'shadow-md');
                            card.classList.add('border', 'border-gray-200', 'bg-gray-50');
                            
                            // Update het nummertje op de tab (als we er één hebben)
                            ...
                            if (tabBadge) {
                                ...
                                if (currentCount > 1) {
                                    ...
                                    tabBadge.remove(); // Verwijder de hele tab-badge als het de laatste was
                                }
                            }
                        }
                    });
            ...
            function confirmDelete(uuid, name) {
                ...
                document.getElementById('confirmDeleteBtn').href = `contactbeheer.php?tab=berichten&delete_message=${uuid}`;
                modal.classList.remove('hidden');
                setTimeout(() => {
